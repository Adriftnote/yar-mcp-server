...
export function heartbeatSession(params: {
  ...
  const existing = db.prepare("SELECT * FROM sessions WHERE session_id = ?").get(params.session_id) as SessionRow | undefined;
  if (!existing) {
...
export function cleanupExpiredSessions(): number {
  ...
  db.transaction(() => {
    ...
    db.prepare(
      `DELETE FROM channel_subscriptions WHERE session_id IN (${placeholders})`
    ).run(...expiredIds);
