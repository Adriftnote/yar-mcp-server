...
import type { JoinInput, SayInput, ListenInput, LeaveInput } from "../schemas/tool-schemas.js";
...
function chat(display: string, meta?: Record<string, unknown>): ToolResult {
  const content: ToolResult["content"] = [{ type: "text", text: display }];
  if (meta) {
    ...
    async (params: JoinInput): Promise<ToolResult> => {
      try {
        ...
        const memberList = result.members
          ...
          .join("\n");
        return chat(
          `🚪 #${result.channel.channel_name} 입장! (${params.nickname})\n\n👥 멤버:\n${memberList}`,
          { session_id: sessionId, channel: result.channel.channel_name }
        );
      } catch (error) {
    ...
    async (params: SayInput): Promise<ToolResult> => {
      try {
        ...
        return chat(
          ...
          { message_id: msg.message_id, channel: params.channel, mentions: msg.mentions }
        );
      } catch (error) {
        return mapToToolError(error);
      }
      ...
      try {
        ...
        while (Date.now() < deadline) {
          ...
          if (messages.length > 0) {
            ...
            return chat(
              chatLines,
              { channel: params.channel, last_id, timed_out: false }
            );
          }
        ...
        // Timed out
        return chat(
          `⏳ ${params.timeout_seconds}초 대기 — 새 메시지 없음`,
          { channel: params.channel, last_id: params.after_id ?? null, timed_out: true }
        );
      } catch (error) {
        return mapToToolError(error);
      }
