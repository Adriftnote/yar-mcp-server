...
app.get("/api/channels", (_req, res) => {
    try {
        const channels = db.prepare("SELECT channel_id, channel_name, description, created_at FROM channels ORDER BY created_at ASC").all();
        const result = channels.map((ch) => {
            const members = db.prepare("SELECT nickname, session_id FROM channel_subscriptions WHERE channel_id = ? ORDER BY subscribed_at ASC").all(ch.channel_id);
            return {
...
app.get("/api/messages", (req, res) => {
    try {
        ...
        const channelRow = db.prepare("SELECT channel_id FROM channels WHERE channel_name = ?").get(channelName);
        if (!channelRow) {
        ...
        if (afterId) {
            const cursorRow = db.prepare("SELECT created_at FROM channel_messages WHERE message_id = ?").get(afterId);
      ...
      LIMIT 100
    `).all(channelRow.channel_id, afterCreatedAt);
...
app.get("/api/members", (req, res) => {
    try {
        ...
        const members = db.prepare("SELECT nickname FROM channel_subscriptions WHERE channel_id = ? ORDER BY subscribed_at ASC").all(channelRow.channel_id);
...
function getOrCreateWebSession(nickname) {
...
function ensureWebUserInChannel(channelId, sessionId, nickname) {
    const existing = db.prepare("SELECT nickname FROM channel_subscriptions WHERE channel_id = ? AND session_id = ?").get(channelId, sessionId);
    if (!existing) {
        db.prepare(`
...
app.post("/api/send", (req, res) => {
    try {
        const { channel, nickname, text } = req.body;
        if (!channel || !nickname || !text) {
            res.status(400).json({ error: "channel, nickname, and text are required" });
            return;
        }
        // Get or create channel
        let channelRow = db.prepare("SELECT channel_id FROM channels WHERE channel_name = ?").get(channel);
        if (!channelRow) {
...
function openBrowser(url) {
    ...
    exec(`${cmd} ${url}`, (err) => {
        if (err)
            console.log(`[yar-monitor] Could not auto-open browser. Visit: ${url}`);
    });
}
// Start server
