...
import { MAX_MESSAGE_SIZE, RATE_LIMIT_PER_SECOND, CHANNEL_MSG_RETAIN_MS, LISTEN_MAX_MESSAGES, } from "../constants.js";
...
function rowToChannelMessage(row) {
    return {
...
function checkRateLimit(sessionId) {
    ...
    if (recent.length >= RATE_LIMIT_PER_SECOND) {
        throw new YarError(ErrorCode.RATE_LIMIT_EXCEEDED, `Rate limit exceeded: max ${RATE_LIMIT_PER_SECOND} messages/second. Wait and retry.`);
    }
    recent.push(now);
...
export function postMessage(params) {
    ...
    if (bodyBytes > MAX_MESSAGE_SIZE) {
        throw new YarError(ErrorCode.VALIDATION_ERROR, `Message body is ${bodyBytes} bytes, exceeding the ${MAX_MESSAGE_SIZE} byte limit (64KB)`);
    }
    ...
    if (!nickname) {
        throw new YarError(ErrorCode.NOT_IN_CHANNEL, `Not in channel '${params.channel_name}'. Use yar_join first.`);
    }
    const db = getDb();
...
export function fetchMessages(params) {
    ...
    if (params.after_id) {
        const cursorRow = db.prepare("SELECT created_at FROM channel_messages WHERE message_id = ?").get(params.after_id);
        if (cursorRow) {
    ...
    const binds = [channelId, params.own_session_id, afterCreatedAt];
...
export function cleanupExpiredChannelMessages() {
    const db = getDb();
    const cutoff = Date.now() - CHANNEL_MSG_RETAIN_MS;
    const result = db.prepare("DELETE FROM channel_messages WHERE created_at < ?").run(cutoff);
